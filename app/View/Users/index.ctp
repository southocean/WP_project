...
<table>
    <thead>
		<tr>
			<th><?php echo $this->Form->checkbox('all', array('name' => 'CheckAll',  'uID' => 'CheckAll')); ?></th>
	...
	<tbody>						
		...
		<?php endif; ?>
			<td><?php echo $this->Form->checkbox('User.uID.'.$user['User']['uID']); ?></td>
			<td><?php echo $this->Html->link( $user['User']['username']  ,   array('action'=>'edit', $user['User']['uID']),array('escape' => false) );?></td>
			...
			<?php
				if( $user['User']['status'] != 0){ 
					echo $this->Html->link(    "Delete", array('action'=>'delete', $user['User']['uID']));}else{
